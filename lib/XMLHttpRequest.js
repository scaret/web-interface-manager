...
XMLHttpRequestManager.prototype.register = function(xhr){
  ...
  self._ee.emit('xhrRegister/' + xhr.id, xhr);
  setTimeout(function(){
    self._ee.emit('xhrAfterRegister', xhr);
    self._ee.emit('xhrAfterRegister/' + xhr.id, xhr);
  }, 0);
};
...
XMLHttpRequest.prototype.open = function(){
  ...
  xhr.onload = function(){
    ...
    xmlHttpRequestManager._ee.emit('xhrBeforeLoad/' + self.id, self);
  ...
  xmlHttpRequestManager._ee.emit('xhrBeforeOpen/' + self.id, self);
...
XMLHttpRequest.prototype.send = function(){
  ...
  if (self.xhr){
    ...
    xmlHttpRequestManager._ee.emit('xhrBeforeSend/' + self.id, self);

    self.xhr.send.apply(self.xhr, self.sendArgs);
    xmlHttpRequestManager._ee.emit('xhrAfterSend', self);
    xmlHttpRequestManager._ee.emit('xhrAfterSend/' + self.id, self);
  }else{
